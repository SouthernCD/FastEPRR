...
hasOtherNucl <- function(currNuclName) {
    ...
    donotHave <- FALSE
    if(!all(currNuclName %in% fourNucl)) {
        donotHave <- TRUE
    }
    return(donotHave)
}
...
getZeroLowHigh <- function(nsam, douton, xton, missDaIDs) {
    ...
    if(is.null(FastEPRRdemoParas)) {
        argv <- c("sim", as.character(nsam), as.character(FastEPRRrepN), "-Y", "0", as.character(douton), as.character(xton), "-U", "0", "-H", "1", as.character(10000), "-r", as.character(0.0), "10000", "-seeds", as.character(seed1))
    } else {
        argv <- c("sim", as.character(nsam), as.character(FastEPRRrepN), "-Y", "0", as.character(douton), as.character(xton), "-U", "0", "-H", "1", as.character(10000), "-r", as.character(0.0), "10000")
    ...
    zeroLow <- zeroHs[round(FastEPRRrepN * 0.025)]
    zeroHigh <- zeroHs[round(FastEPRRrepN * 0.975)]
    rm(zeroHs)
    rm(RCallC)
...
firstModel <- function(nsam, currDou, currXto, missDaIDs) {
    ...
    nr <- 0
    ssAndRho <- matrix(nrow=FastEPRRrepN*length(rhoTrainingSet1), ncol=5)
    
    for(rho in rhoTrainingSet1) {
        cat(rho)
        cat(" ")
        seed1 <- sample(1:100000, 1)
    ...
    firMaxH <- firMaxHs[round(length(firMaxHs)*0.95)]
...
secModel <- function(nsam, currDou, currXto, missDaIDs, firTD) {
    ...
    ssAndRhoSec <- matrix(nrow=(nrow(firTD)+FastEPRRrepN*length(rhoTrainingSet2)), ncol=5) 
    ssAndRhoSec[1:(nr*FastEPRRrepN),] <- firTD
    
    for(rho in rhoTrainingSet2) {
        cat(rho)
        cat(" ")
        seed1 <- sample(1:100000, 1)
        if(is.null(FastEPRRdemoParas)) {
...
convertToZeroOne <- function(mxSrc) {
    ...
    mxSrc <- as.data.frame(mxSrc)
    dealSingleSNP <- function(singleSNP) {
        singleSNP <-  as.vector(singleSNP)
        tempTable <- sort(table(singleSNP))
        nuclCount <- as.vector(tempTable)
        nuclName <- names(tempTable)
        ...
        numOfnucl <- length(nuclName)
        if(numOfnucl == 2) {
            tempRow <- which(singleSNP == nuclName[1])
            singleSNP[tempRow] <- '1' 
            
            tempRow <- which(singleSNP == nuclName[2])
            singleSNP[tempRow] <- '0'
        } else if(numOfnucl == 3) { # multiple hit
            tempRow <- which(singleSNP == nuclName[1])
            singleSNP[tempRow] <- '1'
            ...
            tempRow <- which(singleSNP == nuclName[3])
            singleSNP[tempRow] <- '0'
        } else if(numOfnucl == 4) { # multiple hit
            tempRow <- which(singleSNP == nuclName[1])
            singleSNP[tempRow] <- '1'
            ...
            tempRow <- which(singleSNP == nuclName[4])
            singleSNP[tempRow] <- '0'
        }
        return(singleSNP)
    }
    
    mxSrc <- lapply(mxSrc, dealSingleSNP)
    mxSrc <- matrix(unlist(mxSrc), ncol = seqLength)
    return(mxSrc)
}
