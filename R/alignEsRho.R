alignEsRho <- function(paras) {
    ...
    srcData <- tryCatch({
        read.alignment(file=paras$fp, format=paras$fm, forceToLower = TRUE)
    }, error=function(err) {
    ...
    if(is.numeric(erS) && (erS >= seqLength)) {
        stop(cat("The estimated start position is larger than or equal to sequence length: ", seqLength, "!", sep=""))
    }
    
    if(is.numeric(erE) && (erE >= seqLength)) {
        warning(cat("The estimated end position is larger than or equal to sequence length: ", seqLength, ". FastEPRR use sequence length as erEnd.", sep=""))
    }
    ...
    if(!is.numeric(winL) && !is.numeric(stepL)) {
        alnWinRho(nsam, seqMxPro, pStart, pEnd)
    } else if(is.numeric(winL) && !is.numeric(stepL)) {
