...
// 添加函数声明
void pick2_chrom(int pop, int *config, int *c1, int *c2);
...
void pick2_chrom(int pop, int *config, int *c1, int *c2) {
    int c1_temp, c2_temp, cs, cb, i, count;

    pick2(config[pop], &c1_temp, &c2_temp);
    cs = (c1_temp > c2_temp) ? c2_temp : c1_temp;
    cb = (c1_temp > c2_temp) ? c1_temp : c2_temp;
    i = count = 0;
    for (;;) {
